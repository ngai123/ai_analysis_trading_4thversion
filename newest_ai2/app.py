...
def get_default_parameters():
    ...
    return {
        ...
        'model_path': r"C:\2\Vhack\AI\newest_ai copy\saved_models\1000_bitcoin_model.keras",
        'scaler_path': r"C:\2\Vhack\AI\newest_ai copy\saved_models\1000_scaler.json"
    }

@app.route('/')
def index():
    """Render the main page and automatically load pre-trained model"""
    parameters = get_default_parameters()
    
    # Check if pre-trained model exists using absolute paths
    pretrained_model_path = r"C:\2\Vhack\AI\newest_ai copy\saved_models\1000_bitcoin_model.keras"
    pretrained_scaler_path = r"C:\2\Vhack\AI\newest_ai copy\saved_models\1000_scaler.json"
    has_pretrained_model = os.path.exists(pretrained_model_path) and os.path.exists(pretrained_scaler_path)
    
    # Automatically load pre-trained model on startup if it exists
    if has_pretrained_model and pretrained_model is None:
        load_pretrained_model_internal()
...
def train_model_task(task_id, parameters):
    """Background task to train the model - kept for compatibility"""
    try:
        # Update task status
...
def train_model():
    """Start a model training task - kept for compatibility"""
    data = request.get_json()
...
def load_pretrained_model_internal():
    """Internal function to load the pre-trained model"""
...
def load_pretrained_model():
    ...
    if pretrained_model is not None:
        ...
        return jsonify({
            'status': 'success',
            'task_id': task_id,
            'message': 'Pre-trained model already loaded',
            'simulation_results': pretrained_model['simulation_results'],
            'performance_plots': pretrained_model['performance_plots']
        })
    
    # Otherwise, load it
    result = load_pretrained_model_internal()
